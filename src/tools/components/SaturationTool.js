...
export default function SaturationTool({ id, initialSaturation = 0 }) {
  const { canvasEditor, imageVersion } = useImageContext();
  const originalRef = useRef(null);
  const isDraggingRef = useRef(false);
  const [saturation, setSaturation] = useState(initialSaturation); // -100..100
  ...
  const applyAdjustment = useCallback((satPct) => {
    ...
    const satAdj = Math.max(-100, Math.min(100, satPct)) / 100; // -1..1

    for (let i = 0; i < d.length; i += 4) {
      const r = d[i], g = d[i + 1], b = d[i + 2];
      let [h, s, l] = rgbToHsl(r, g, b);
      s = Math.min(1, Math.max(0, s + satAdj * (1 - Math.abs(s))));
